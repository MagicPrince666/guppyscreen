...
ConsolePanel::ConsolePanel(GcodeTransmitClient &websocket_client, std::mutex &lock, lv_obj_t *parent)
    : gcode_transmit_(websocket_client), lv_lock(lock), console_cont(lv_obj_create(parent)), top_cont(lv_obj_create(console_cont)), output(lv_textarea_create(top_cont)), macro_list(lv_table_create(top_cont)), input_cont(lv_obj_create(console_cont)), input(lv_textarea_create(input_cont)), kb(lv_keyboard_create(console_cont))
{
    ...
    if (code == LV_EVENT_READY) {
        ...
        if (!history.empty()) {
            ...
            if (front != std::string(cmd)) {
                ...
                gcode_transmit_.send_jsonrpc("server.database.post_item", h);
            }
        }
