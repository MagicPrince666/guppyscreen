...
FanPanel::FanPanel(GcodeTransmitClient &websocket_client, std::mutex &lock)
    : NotifyConsumer(lock), gcode_transmit_(websocket_client), fanpanel_cont(lv_obj_create(lv_scr_act())), fans_cont(lv_obj_create(fanpanel_cont)), back_btn(fanpanel_cont, &back, "Back", &FanPanel::_handle_callback, this)
{
    ...
    if (lv_event_get_code(event) == LV_EVENT_RELEASED) {
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_slider()) {
                ...
                gcode_transmit_.gcode_script(fmt::format(fmt::format("SET_PIN PIN={} VALUE={}", fan_name, pct)));
                break;
            }
        }
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_off()) {
                ...
                gcode_transmit_.gcode_script(fmt::format("SET_PIN PIN={} VALUE=0", fan_name));
                f.second->update_value(0);
                break;
            } else if (obj == f.second->get_max()) {
                std::string fan_name = KUtils::get_obj_name(f.first);
                spdlog::trace("turning fan to max {}", fan_name);
                gcode_transmit_.gcode_script(fmt::format("SET_PIN PIN={} VALUE=255", fan_name));
...
void FanPanel::handle_fan_update_part_fan(lv_event_t *event)
{
    ...
    if (lv_event_get_code(event) == LV_EVENT_RELEASED) {
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_slider()) {
                gcode_transmit_.gcode_script(fmt::format(fmt::format("M106 S{}", pct)));
                break;
            }
        }
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_off()) {
                gcode_transmit_.gcode_script("M106 S0");
                f.second->update_value(0);
                break;
            } else if (obj == f.second->get_max()) {
                gcode_transmit_.gcode_script("M106 S255");
                f.second->update_value(100);
                break;
            }
        }
    ...
    if (lv_event_get_code(event) == LV_EVENT_RELEASED) {
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_slider()) {
                std::string fan_name = KUtils::get_obj_name(f.first);
                spdlog::trace("update fan {}", fan_name);
                gcode_transmit_.gcode_script(fmt::format(fmt::format("SET_FAN_SPEED FAN={} SPEED={}", fan_name, pct)));
                break;
            }
        }
    } else if (lv_event_get_code(event) == LV_EVENT_CLICKED) {
        ...
        for (auto &f : fans) {
            if (obj == f.second->get_off()) {
                std::string fan_name = KUtils::get_obj_name(f.first);
                spdlog::trace("turning off fan {}", fan_name);
                gcode_transmit_.gcode_script(fmt::format("SET_FAN_SPEED FAN={} SPEED=0", fan_name));
                f.second->update_value(0);
                break;
            } else if (obj == f.second->get_max()) {
                std::string fan_name = KUtils::get_obj_name(f.first);
                spdlog::trace("turning fan to max {}", fan_name);
                gcode_transmit_.gcode_script(fmt::format("SET_FAN_SPEED FAN={} SPEED=1", fan_name));
                f.second->update_value(100);
                break;
            }
        }
