...
MacroItem::MacroItem(GcodeTransmitClient &c,
                     lv_obj_t *parent,
                     ...
                     // lv_color_t bg_color,
                     bool hide)
    : gcode_transmit_(c), cont(lv_obj_create(parent)), top_cont(lv_obj_create(cont)), macro_label(lv_label_create(top_cont)), hide_show_cont(lv_obj_create(top_cont)), hide_show(lv_label_create(hide_show_cont)), kb(keyboard), hidden(hide), always_visible(false)
{
    if (hidden) {
...
void MacroItem::handle_send_macro(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        gcode_transmit_.gcode_script(fmt::format("{}", fmt::join(kv, " ")));
    }
}
