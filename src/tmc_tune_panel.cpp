...
AutoTmcContainer::AutoTmcContainer(const std::list<std::string> &motors,
                                   ...
                                   lv_obj_t *parent)
    : cont(lv_obj_create(parent)), name(stepper_name), motors_dd(lv_dropdown_create(cont)), tuning_goal_dd(lv_dropdown_create(cont)), spinbox_cont(lv_obj_create(cont)), sensorless_threshold(lv_spinbox_create(spinbox_cont)), configured_motor_idx(motor_idx), configured_goal_idx(goal_idx), has_sg(sg_configured), configured_sensorless_thrs(sgthrs), sg_range(sg_min_max)
{
...
TmcTunePanel::TmcTunePanel(GcodeTransmitClient &c)
    : gcode_transmit_(c), cont(lv_obj_create(lv_scr_act())), controls_cont(lv_obj_create(cont)), btns_cont(lv_obj_create(cont)), save_btn(btns_cont, &sd_img, "Save/Restart", [](lv_event_t *e) {
    TmcTunePanel *panel = (TmcTunePanel*)e->user_data;
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
      panel->save_config();
    } }, this), back_btn(btns_cont, &back, "Back", [](lv_event_t *e) {
    TmcTunePanel *panel = (TmcTunePanel*)e->user_data;
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
      panel->background();
    } }, this)
...
void TmcTunePanel::init(json &j, fs::path &kp)
{
    ...
    for (auto &s : configured_steppers) {
        ...
        bool has_virtual_endstop = !endstop_pin.is_null() && endstop_pin.template get<std::string>().find("virtual_endstop") != std::string::npos && sg_range_el != tmc_sg_range.end();
...
void TmcTunePanel::save_config()
{
    ...
    gcode_transmit_.gcode_script(fmt::format("{}\nSAVE_CONFIG", fmt::join(save_configs, "\n")));
}
