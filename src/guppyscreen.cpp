...
GuppyScreen::GuppyScreen()
    : spoolman_panel(gcode_transmit_, lv_lock), main_panel(gcode_transmit_, lv_lock, spoolman_panel), init_panel(main_panel, main_panel.get_tune_panel().get_bedmesh_panel(), lv_lock)
{
...
GuppyScreen *GuppyScreen::init(std::function<void(lv_color_t, lv_color_t)> hal_init)
{
    ...
    gcode_transmit_.register_notify_update(State::get_instance());
...
void GuppyScreen::connect_ws(const std::string &url)
{
    ...
    int fd = gcode_transmit_.connect(url.c_str(), [this]() { init_panel.connected(gcode_transmit_); }, [this]() { init_panel.disconnected(gcode_transmit_); });
}
