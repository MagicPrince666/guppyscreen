...
MainPanel::MainPanel(GcodeTransmitClient &websocket,
                     std::mutex &lock,
                     SpoolmanPanel &sm)
    : NotifyConsumer(lock), gcode_transmit_(websocket), homing_panel(gcode_transmit_, lock), fan_panel(gcode_transmit_, lock), led_panel(gcode_transmit_, lock), tabview(lv_tabview_create(lv_scr_act(), LV_DIR_LEFT, 60)), main_tab(lv_tabview_add_tab(tabview, HOME_SYMBOL)), macros_tab(lv_tabview_add_tab(tabview, MACROS_SYMBOL)), macros_panel(gcode_transmit_, lock, macros_tab), console_tab(lv_tabview_add_tab(tabview, CONSOLE_SYMBOL)), console_panel(gcode_transmit_, lock, console_tab), printertune_tab(lv_tabview_add_tab(tabview, TUNE_SYMBOL)), setting_tab(lv_tabview_add_tab(tabview, SETTING_SYMBOL)), setting_panel(websocket, lock, setting_tab, sm), main_cont(lv_obj_create(main_tab)), print_status_panel(websocket, lock, main_cont), print_panel(gcode_transmit_, lock, print_status_panel), printertune_panel(gcode_transmit_, lock, printertune_tab, print_status_panel.get_finetune_panel()), numpad(Numpad(main_cont)), extruder_panel(gcode_transmit_, lock, numpad, sm), prompt_panel(websocket, lock, main_cont), spoolman_panel(sm), temp_cont(lv_obj_create(main_cont)), temp_chart(lv_chart_create(main_cont)), homing_btn(main_cont, &move, "Homing", &MainPanel::_handle_homing_cb, this), extrude_btn(main_cont, &filament_img, "Extrude", &MainPanel::_handle_extrude_cb, this), action_btn(main_cont, &fan, "Fans", &MainPanel::_handle_fanpanel_cb, this), led_btn(main_cont, &light_img, "LED", &MainPanel::_handle_ledpanel_cb, this), print_btn(main_cont, &print, "Print", &MainPanel::_handle_print_cb, this)
{
    lv_style_init(&style);
...
void MainPanel::subscribe()
{
    ...
    // gcode_transmit_.send_jsonrpc("objects/query", [this](json &d) { console_panel.handle_macros(d); });
...
void MainPanel::create_sensors(json &temp_sensors)
{
    ...
    for (auto &sensor : temp_sensors.items()) {
        ...
        sensors.insert({key, std::make_shared<SensorContainer>(gcode_transmit_, temp_cont, sensor_img, 150,
