...
SpoolmanPanel::SpoolmanPanel(GcodeTransmitClient &c, std::mutex &l)
    : gcode_transmit_(c), lv_lock(l), cont(lv_obj_create(lv_scr_act())), spool_table(lv_table_create(cont)), controls(lv_obj_create(cont)), switch_cont(lv_obj_create(controls)), show_archived(lv_switch_create(switch_cont)), reload_btn(controls, &refresh_img, "Reload", &SpoolmanPanel::_handle_callback, this), back_btn(controls, &back, "Back", &SpoolmanPanel::_handle_callback, this), active_id(-1), sorted_by(SORTED_BY_ID)
{
    ...
    gcode_transmit_.register_method_callback("notify_active_spool_set",
                                             "SpoolmanPanel",
...
void SpoolmanPanel::init()
{
    ...
    gcode_transmit_.send_jsonrpc("server.spoolman.proxy", param, [this](json &d) {
    ...
    gcode_transmit_.send_jsonrpc("server.spoolman.get_spool_id", [this](json &d) {
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (row != 0) {
            if (col == 6 && selected != NULL && strlen(selected) != 0 && std::memcmp(LV_SYMBOL_PLAY, selected, 3) == 0) {
                ...
                gcode_transmit_.send_jsonrpc("server.spoolman.post_spool_id", param);
            }
