...
BedMeshPanel::BedMeshPanel(GcodeTransmitClient &c, std::mutex &l)
    : NotifyConsumer(l), gcode_transmit_(c), cont(lv_obj_create(lv_scr_act())), prompt(lv_obj_create(lv_scr_act())), top_cont(lv_obj_create(cont)), mesh_table(lv_table_create(top_cont)), profile_cont(lv_table_create(top_cont)), profile_table(lv_table_create(profile_cont)), profile_info(lv_table_create(profile_cont)), controls_cont(lv_obj_create(cont)), save_btn(controls_cont, &sd_img, "Save Profile", &BedMeshPanel::_handle_callback, this), clear_btn(controls_cont, &delete_img, "Clear Profile", &BedMeshPanel::_handle_callback, this), calibrate_btn(controls_cont, &bedmesh_img, "Calibrate", &BedMeshPanel::_handle_callback, this), back_btn(controls_cont, &back, "Back", &BedMeshPanel::_handle_callback, this), msgbox(lv_obj_create(prompt)), input(lv_textarea_create(msgbox)), kb(lv_keyboard_create(prompt))
{
...
void BedMeshPanel::handle_callback(lv_event_t *event)
{
    ...
    if (btn == save_btn.get_container()) {
        ...
        gcode_transmit_.gcode_script("BED_MESH_CLEAR");
        ...
        if (!v.is_null()) {
            if (v.template get<std::string>() == "xy") {
                gcode_transmit_.gcode_script("BED_MESH_CALIBRATE");
                return;
            }
        }
...
void BedMeshPanel::handle_profile_action(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED) {
        ...
        if (col == 2) {
            ...
            gcode_transmit_.gcode_script(fmt::format("BED_MESH_PROFILE REMOVE=\"{}\"\nSAVE_CONFIG", profile_name));
        } else if (col == 1 && selected != NULL && strlen(selected) != 0) {
            // load profile
            spdlog::trace("selected {}, load mesh {}", strlen(selected), profile_name);
            gcode_transmit_.gcode_script(fmt::format("BED_MESH_PROFILE LOAD=\"{}\"", profile_name));
...
void BedMeshPanel::handle_prompt_save(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        gcode_transmit_.gcode_script(fmt::format("BED_MESH_PROFILE SAVE=\"{}\"\nSAVE_CONFIG", profile_name));
