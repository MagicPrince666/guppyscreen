...
InitPanel::InitPanel(MainPanel &mp, BedMeshPanel &bmp, std::mutex &l)
    : cont(lv_obj_create(lv_scr_act())), label(lv_label_create(cont)), main_panel(mp), bedmesh_panel(bmp), lv_lock(l)
{
...
void InitPanel::connected(GcodeTransmitClient &ws)
{
...
void InitPanel::disconnected(GcodeTransmitClient &ws)
{
