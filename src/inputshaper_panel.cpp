...
InputShaperPanel::InputShaperPanel(GcodeTransmitClient &c, std::mutex &l)
    : gcode_transmit_(c), lv_lock(l), cont(lv_obj_create(lv_scr_act()))

      // xgraph
      ,
      xgraph_cont(lv_obj_create(cont)), xgraph(lv_img_create(xgraph_cont)), xoutput(lv_label_create(cont)), xspinner(lv_spinner_create(cont, 1000, 60))

      // ygraph
      ,
      ygraph_cont(lv_obj_create(cont)), ygraph(lv_img_create(ygraph_cont)), youtput(lv_label_create(cont)), yspinner(lv_spinner_create(cont, 1000, 60))

      // x controls
      ,
      xcontrol(lv_obj_create(cont)), xaxis_label(lv_label_create(xcontrol)), x_switch(lv_switch_create(xcontrol)), xslider_cont(lv_obj_create(xcontrol)), xslider(lv_slider_create(xslider_cont)), xlabel(lv_label_create(xslider_cont)), xshaper_dd(lv_dropdown_create(xcontrol))

      // y controls
      ,
      ycontrol(lv_obj_create(cont)), yaxis_label(lv_label_create(ycontrol)), y_switch(lv_switch_create(ycontrol)), yslider_cont(lv_obj_create(ycontrol)), yslider(lv_slider_create(yslider_cont)), ylabel(lv_label_create(yslider_cont)), yshaper_dd(lv_dropdown_create(ycontrol))

      ,
      button_cont(lv_obj_create(cont)), switch_cont(lv_obj_create(button_cont)), graph_switch_label(lv_label_create(switch_cont)), graph_switch(lv_switch_create(switch_cont)), calibrate_btn(button_cont, &resume, "Calibrate", &InputShaperPanel::_handle_callback, this), save_btn(button_cont, &sd_img, "Save", &InputShaperPanel::_handle_callback, this), emergency_btn(button_cont, &emergency, "Stop", &InputShaperPanel::_handle_callback, this,
                                                                                                                                                                                                                                                                                                                                                                              "Do you want to emergency stop?",
                                                                                                                                                                                                                                                                                                                                                                              [&c]() {
                                                                                                                                                                                                                                                                                                                                                                                  spdlog::debug("emergency stop pressed");
                                                                                                                                                                                                                                                                                                                                                                                  c.send_jsonrpc("printer.emergency_stop");
                                                                                                                                                                                                                                                                                                                                                                              }),
      back_btn(cont, &back, "Back", &InputShaperPanel::_handle_callback, this), ximage_fullsized(false), yimage_fullsized(false)
{
    ...
    // gcode_transmit_.register_gcode_resp([this](json& d) { this->handle_macro_response(d); });
    gcode_transmit_.register_method_callback("notify_gcode_response",
                                             "InputShaperPanel",
...
void InputShaperPanel::handle_callback(lv_event_t *event)
{
    ...
    if (btn == calibrate_btn.get_container()) {
        ...
        if ((x_requested || y_requested) && !KUtils::is_homed()) {
            gcode_transmit_.gcode_script("G28");
        }

        if (x_requested) {
            // gcode_transmit_.gcode_script(fmt::format("TEST_RESONANCES AXIS=X NAME=x FREQ_START={} FREQ_END={}\nM400", 5, 10));
            gcode_transmit_.gcode_script(fmt::format("TEST_RESONANCES AXIS=X NAME=x\nM400"));
        ...
        if (y_requested) {
            // gcode_transmit_.gcode_script(fmt::format("TEST_RESONANCES AXIS=Y NAME=y FREQ_START={} FREQ_END={}\nM400", 5, 10));
            gcode_transmit_.gcode_script(fmt::format("TEST_RESONANCES AXIS=Y NAME=y\nM400"));
        ...
        gcode_transmit_.gcode_script(fmt::format("SAVE_INPUT_SHAPER SHAPER_FREQ_X={} SHAPER_TYPE_X={} SHAPER_FREQ_Y={} SHAPER_TYPE_Y={}\nSAVE_CONFIG",
                                                 xhz, xbuf, yhz, ybuf));

    } else if (btn == back_btn.get_container()) {
        lv_obj_move_background(cont);
    } else if (btn == emergency_btn.get_container()) {
        gcode_transmit_.send_jsonrpc("printer.emergency_stop");
    }
}
...
void InputShaperPanel::handle_macro_response(json &j)
{
    ...
    if (!v.is_null()) {
        ...
        if (resp.rfind("// {\"shapers\":", 0) == 0) {
            ...
            gcode_transmit_.gcode_script(fmt::format("RUN_SHELL_COMMAND CMD=guppy_input_shaper PARAMS={:?}", arg));
