...
LimitsPanel::LimitsPanel(GcodeTransmitClient &c, std::mutex &l)
    : NotifyConsumer(l), gcode_transmit_(c), cont(lv_obj_create(lv_scr_act())), limit_cont(lv_obj_create(cont)), velocity(limit_cont, "Velocity (mm/s)", &refresh_img, "Reset", &refresh_img, NULL, &LimitsPanel::_handle_callback, this, ""), acceleration(limit_cont, "Acceleration (mm/s2)", &refresh_img, "Reset", &refresh_img, NULL, &LimitsPanel::_handle_callback, this, ""), square_corner(limit_cont, "Square Corner Velocity (mm/s)", &refresh_img, "Reset", &refresh_img, NULL, &LimitsPanel::_handle_callback, this, ""), accel_to_decel(limit_cont, "Acceleration to Deceleration (mm/s2)", &refresh_img, "Reset", &refresh_img, NULL, &LimitsPanel::_handle_callback, this, ""), back_btn(cont, &back, "Back", &LimitsPanel::_handle_callback, this), max_velocity_default(1000), max_accel_default(20000), max_accel_to_decel_default(10000), square_corner_default(5)
{
...
void LimitsPanel::handle_callback(lv_event_t *e)
{
    ...
    if (lv_event_get_code(e) == LV_EVENT_RELEASED) {
        ...
        if (obj == velocity.get_slider()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT VELOCITY={}", v));

        } else if (obj == acceleration.get_slider()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT ACCEL={}", v));

        } else if (obj == accel_to_decel.get_slider()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT ACCEL_TO_DECEL={}", v));

        } else if (obj == square_corner.get_slider()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT SQUARE_CORNER_VELOCITY={}", v));
        }

    } else if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        if (btn == velocity.get_off()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT VELOCITY={}", max_velocity_default));

        } else if (btn == acceleration.get_off()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT ACCEL={}", max_accel_default));

        } else if (btn == accel_to_decel.get_off()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT ACCEL_TO_DECEL={}", max_accel_to_decel_default));

        } else if (btn == square_corner.get_off()) {
            gcode_transmit_.gcode_script(fmt::format("SET_VELOCITY_LIMIT SQUARE_CORNER_VELOCITY={}", square_corner_default));
        }
    }
}
