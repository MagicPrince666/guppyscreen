...
ExtruderPanel::ExtruderPanel(GcodeTransmitClient &websocket_client,
                             std::mutex &lock,
                             Numpad &numpad,
                             SpoolmanPanel &sm)
    : NotifyConsumer(lock), gcode_transmit_(websocket_client), panel_cont(lv_obj_create(lv_scr_act())), spoolman_panel(sm), extruder_temp(gcode_transmit_, panel_cont, &extruder, 150,
      ...
      rightside_btns_cont(lv_obj_create(panel_cont)), leftside_btns_cont(lv_obj_create(panel_cont)), load_btn(leftside_btns_cont, &load_filament_img, "Load", &ExtruderPanel::_handle_callback, this), unload_btn(leftside_btns_cont, &unload_filament_img, "Unload", &ExtruderPanel::_handle_callback, this), cooldown_btn(leftside_btns_cont, &cooldown_img, "Cooldown", &ExtruderPanel::_handle_callback, this), spoolman_btn(rightside_btns_cont, &spoolman_img, "Spoolman", &ExtruderPanel::_handle_callback, this), extrude_btn(rightside_btns_cont, &extrude_img, "Extrude", &ExtruderPanel::_handle_callback, this), retract_btn(rightside_btns_cont, &retract_img, "Retract", &ExtruderPanel::_handle_callback, this), back_btn(rightside_btns_cont, &back, "Back", &ExtruderPanel::_handle_callback, this), load_filament_macro("LOAD_FILAMENT"), unload_filament_macro("UNLOAD_FILAMENT"), cooldown_macro("SET_HEATER_TEMPERATURE HEATER=extruder TARGET=0")
{
...
void ExtruderPanel::handle_callback(lv_event_t *e)
{
    ...
    if (lv_event_get_code(e) == LV_EVENT_VALUE_CHANGED) {
        ...
        if (btn == extrude_btn.get_container()) {
            ...
            gcode_transmit_.gcode_script(fmt::format("M109 S{}\nM83\nG1 E{} F{}", temp, len, std::stoi(speed) * 60));
        }
        ...
        if (btn == unload_btn.get_container()) {
            if (unload_filament_macro == "_GUPPY_QUIT_MATERIAL") {
                const char *temp = lv_btnmatrix_get_btn_text(temp_selector.get_selector(),
                                                             temp_selector.get_selected_idx());
                gcode_transmit_.gcode_script(fmt::format("{} EXTRUDER_TEMP={}", unload_filament_macro, temp));
            } else {
                gcode_transmit_.gcode_script(unload_filament_macro);
            }
        }
        ...
        if (btn == load_btn.get_container()) {
            if (load_filament_macro == "_GUPPY_LOAD_MATERIAL") {
                ...
                const char *len  = lv_btnmatrix_get_btn_text(length_selector.get_selector(),
                                                             length_selector.get_selected_idx());
                gcode_transmit_.gcode_script(fmt::format("{} EXTRUDER_TEMP={} EXTRUDE_LEN={}", load_filament_macro, temp, len));
            } else {
                gcode_transmit_.gcode_script(load_filament_macro);
            }
        }

        if (btn == cooldown_btn.get_container()) {
            gcode_transmit_.gcode_script(cooldown_macro);
        }
