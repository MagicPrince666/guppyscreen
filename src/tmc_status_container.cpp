...
TmcStatusContainer::TmcStatusContainer(GcodeTransmitClient &c,
                                       lv_obj_t *parent,
                                       const std::string &stepper_name)
    : gcode_transmit_(c), cont(lv_obj_create(parent)), chart_cont(lv_obj_create(cont)), label(lv_label_create(chart_cont)), legend(lv_obj_create(chart_cont)), chart(lv_chart_create(chart_cont)), sg_series(lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_ORANGE), LV_CHART_AXIS_PRIMARY_Y)), irms_series(lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_RED), LV_CHART_AXIS_PRIMARY_Y)), semin_series(lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_BLUE), LV_CHART_AXIS_PRIMARY_Y)), semax_series(lv_chart_add_series(chart, lv_palette_main(LV_PALETTE_GREEN), LV_CHART_AXIS_PRIMARY_Y)), stepper_config(lv_obj_create(cont))
...
void TmcStatusContainer::update_tmc_value(const std::string &stepper_name,
                                          ...
                                          int value)
{
    gcode_transmit_.gcode_script(fmt::format("SET_TMC_FIELD field={} value={} stepper={}",
