...
PrintStatusPanel::PrintStatusPanel(GcodeTransmitClient &websocket_client,
                                   std::mutex &lock,
                                   lv_obj_t *mini_parent)
    : NotifyConsumer(lock), gcode_transmit_(websocket_client), finetune_panel(websocket_client, lock), mini_print_status(mini_parent, &PrintStatusPanel::_handle_callback, this), status_cont(lv_obj_create(lv_scr_act())), buttons_cont(lv_obj_create(status_cont)), finetune_btn(buttons_cont, &fine_tune_img, "Fine Tune", &PrintStatusPanel::_handle_callback, this), pause_btn(buttons_cont, &pause_img, "Pause", &PrintStatusPanel::_handle_callback, this), resume_btn(buttons_cont, &resume, "Resume", &PrintStatusPanel::_handle_callback, this), cancel_btn(buttons_cont, &cancel, "Cancel", &PrintStatusPanel::_handle_callback, this,
      ...
      back_btn(buttons_cont, &back, "Back", &PrintStatusPanel::_handle_callback, this), thumbnail_cont(lv_obj_create(status_cont)), thumbnail(lv_img_create(thumbnail_cont)), pbar_cont(lv_obj_create(thumbnail_cont)), progress_bar(lv_bar_create(pbar_cont)), progress_label(lv_label_create(pbar_cont)), detail_cont(lv_obj_create(status_cont)), extruder_temp(detail_cont, &extruder, 100, "20"), bed_temp(detail_cont, &bed, 100, "21"), print_speed(detail_cont, &speed_up_img, 100, "0 mm/s"), z_offset(detail_cont, &home_z, 100, "0.0 mm"), flow_rate(detail_cont, &extrude, 100, "0.0 mm3/s"), layers(detail_cont, &layers_img, 100, "..."), fan0(detail_cont, &fan, 100, "0%"), elapsed(detail_cont, &clock_img, 100, "0s"), time_left(detail_cont, &hourglass, 100, "..."), estimated_time_s(0), filament_diameter(1.75) // XXX: check config
      ,
      extruder_target(-1), heater_bed_target(-1)
{
...
void PrintStatusPanel::populate()
{
    ...
    if (!printfile.is_null()) {
        ...
        if (fname.length() > 0) {
            ...
            gcode_transmit_.send_jsonrpc("server.files.metadata", fname_input,
...
void PrintStatusPanel::handle_callback(lv_event_t *event)
{
    ...
    if (btn == back_btn.get_container()) {
        ...
        gcode_transmit_.send_jsonrpc("printer.print.pause");
        pause_btn.disable();

    } else if (btn == resume_btn.get_container()) {
        gcode_transmit_.send_jsonrpc("printer.print.resume");
        resume_btn.disable();
    } else if (btn == cancel_btn.get_container()) {
        gcode_transmit_.send_jsonrpc("printer.print.cancel");
...
int PrintStatusPanel::current_layer(json &info)
{
    ...
    if (!current_file.is_null()) {
        ...
        if (!pd.is_null() && pd.template get<int>() > 0 && !zpos.is_null() && !first_layer_height.is_null() && !layer_height.is_null()) {
