...
MacrosPanel::MacrosPanel(GcodeTransmitClient &c, std::mutex &l, lv_obj_t *parent)
    : gcode_transmit_(c), lv_lock(l), cont(lv_obj_create(parent)), top_controls(lv_obj_create(cont)), show_hide_switch(lv_switch_create(top_controls)), top_cont(lv_obj_create(cont)), kb(lv_keyboard_create(cont))
{
...
void MacrosPanel::populate()
{
    ...
    if (!config_json.is_null()) {
        ...
        for (auto const &[k, v] : macros) {
            ...
            macro_items.push_back(std::make_shared<MacroItem>(gcode_transmit_, top_cont, k, v, kb, hidden));

            // if (i % 2 == 0) {
            // 	macro_items.push_back(std::make_shared<MacroItem>(gcode_transmit_, top_cont, k, v, kb, mixed, hidden));
            // } else {
            // 	macro_items.push_back(std::make_shared<MacroItem>(gcode_transmit_, top_cont, k, v, kb, cur_bg, hidden));
            // }
            // i++;
        }
    }
