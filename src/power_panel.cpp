...
PowerPanel::PowerPanel(GcodeTransmitClient &websocket_client, std::mutex &l)
    : gcode_transmit_(websocket_client), lv_lock(l), cont(lv_obj_create(lv_scr_act())), back_btn(cont, &back, "Back", &PowerPanel::_handle_callback, this)
{
...
void PowerPanel::foreground()
{
    ...
    gcode_transmit_.send_jsonrpc("machine.device_power.status", params, [this](json &j) {
    ...
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        for (auto &device : devices) {
            if (obj == device.second) {
                ...
                gcode_transmit_.send_jsonrpc("machine.device_power.post_device", params, [this](json &j) {
                    this->handle_device_callback(j);
                });
                break;
            }
