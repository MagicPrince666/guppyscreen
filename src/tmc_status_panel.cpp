...
TmcStatusPanel::TmcStatusPanel(GcodeTransmitClient &c,
                               std::mutex &lock)
    : NotifyConsumer(lock), gcode_transmit_(c), cont(lv_obj_create(lv_scr_act())), top(lv_obj_create(cont)), toggle(lv_switch_create(top)), back_btn(cont, &back, "Back", [](lv_event_t *e) {
    ...
    } }, this)
{
	...
	p->gcode_transmit_.gcode_script("_GUPPY_LOAD_MODULE SECTION=tmcstatus");
      } else {
	p->gcode_transmit_.gcode_script("_GUPPY_UNLOAD_MODULE SECTION=tmcstatus");
      }
    ...
    if (!tmc_status.is_null()) {
        ...
        for (auto &el : tmc_status.items()) {
            ...
            if (s != metrics.end()) {
                ...
                auto tmc_status_cont = std::make_shared<TmcStatusContainer>(gcode_transmit_, cont, el.key());
