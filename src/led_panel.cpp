...
LedPanel::LedPanel(GcodeTransmitClient &websocket_client, std::mutex &lock)
    : NotifyConsumer(lock), gcode_transmit_(websocket_client), ledpanel_cont(lv_obj_create(lv_scr_act())), leds_cont(lv_obj_create(ledpanel_cont)), back_btn(ledpanel_cont, &back, "Back", &LedPanel::_handle_callback, this)
{
    ...
    if (lv_event_get_code(event) == LV_EVENT_RELEASED) {
        ...
        for (auto &l : leds) {
            if (obj == l.second->get_slider()) {
                ...
                gcode_transmit_.gcode_script(fmt::format(fmt::format("SET_PIN PIN={} VALUE={}", led_name, pct)));
                break;
            }
        }
        ...
        for (auto &l : leds) {
            if (obj == l.second->get_off()) {
                ...
                gcode_transmit_.gcode_script(fmt::format("SET_PIN PIN={} VALUE=0", led_name));
                l.second->update_value(0);
                break;
            } else if (obj == l.second->get_max()) {
                std::string led_name = KUtils::get_obj_name(l.first);
                spdlog::trace("turning led to max {}", led_name);
                gcode_transmit_.gcode_script(fmt::format("SET_PIN PIN={} VALUE=1", led_name));
    ...
    if (lv_event_get_code(event) == LV_EVENT_RELEASED) {
        ...
        for (auto &l : leds) {
            if (obj == l.second->get_slider()) {
                std::string led_name = KUtils::get_obj_name(l.first);
                spdlog::trace("update led {}", led_name);
                gcode_transmit_.gcode_script(fmt::format(fmt::format("SET_LED LED={} WHITE={}", led_name, pct)));
                break;
            }
        }
    } else if (lv_event_get_code(event) == LV_EVENT_CLICKED) {
        ...
        for (auto &l : leds) {
            if (obj == l.second->get_off()) {
                std::string led_name = KUtils::get_obj_name(l.first);
                spdlog::trace("turning off led {}", led_name);
                gcode_transmit_.gcode_script(fmt::format("SET_LED LED={} WHITE=0", led_name));
                l.second->update_value(0);
                break;
            } else if (obj == l.second->get_max()) {
                std::string led_name = KUtils::get_obj_name(l.first);
                spdlog::trace("turning led to max {}", led_name);
                gcode_transmit_.gcode_script(fmt::format("SET_LED LED={} WHITE=1", led_name));
                l.second->update_value(100);
                break;
            }
        }
