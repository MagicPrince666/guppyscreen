...
FineTunePanel::FineTunePanel(GcodeTransmitClient &websocket_client, std::mutex &l)
    : NotifyConsumer(l), gcode_transmit_(websocket_client), panel_cont(lv_obj_create(lv_scr_act())), values_cont(lv_obj_create(panel_cont)), zreset_btn(panel_cont, &refresh_img, "Reset Z", &FineTunePanel::_handle_zoffset, this), zup_btn(panel_cont, &z_closer, "Z+", &FineTunePanel::_handle_zoffset, this), zdown_btn(panel_cont, &z_farther, "Z-", &FineTunePanel::_handle_zoffset, this), pareset_btn(panel_cont, &refresh_img, "Reset PA", &FineTunePanel::_handle_pa, this), paup_btn(panel_cont, &pa_plus_img, "PA+", &FineTunePanel::_handle_pa, this), padown_btn(panel_cont, &pa_minus_img, "PA-", &FineTunePanel::_handle_pa, this), speed_reset_btn(panel_cont, &refresh_img, "Speed Reset", &FineTunePanel::_handle_speed, this), speed_up_btn(panel_cont, &speed_up_img, "Speed+", &FineTunePanel::_handle_speed, this), speed_down_btn(panel_cont, &speed_down_img, "Speed-", &FineTunePanel::_handle_speed, this), flow_reset_btn(panel_cont, &refresh_img, "Flow Reset", &FineTunePanel::_handle_flow, this), flow_up_btn(panel_cont, &flow_up_img, "Flow+", &FineTunePanel::_handle_flow, this), flow_down_btn(panel_cont, &flow_down_img, "Flow-", &FineTunePanel::_handle_flow, this), back_btn(panel_cont, &back, "Back", &FineTunePanel::_handle_callback, this), zoffset_selector(panel_cont, "Z (mm) - PA (mm/s)",
      ...
      z_offset(values_cont, &home_z, 150, 100, 15, "0.0 mm"), pa(values_cont, &pa_plus_img, 150, 100, 15, "0.0 mm/s"), speed_factor(values_cont, &speed_up_img, 150, 100, 15, "100%"), flow_factor(values_cont, &flow_up_img, 150, 100, 15, "100%")
{
...
void FineTunePanel::foreground()
{
    ...
    if (!v.is_null()) {
        speed_factor.update_label(fmt::format("{}%",
                                              static_cast<int>(v.template get<double>() * 100))
                                      .c_str());
    }
...
void FineTunePanel::handle_zoffset(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        if (btn == zreset_btn.get_container()) {
            ...
            gcode_transmit_.gcode_script("SET_GCODE_OFFSET Z=0 MOVE=1");
        } else {
            ...
            gcode_transmit_.gcode_script(fmt::format("SET_GCODE_OFFSET Z_ADJUST={}{} MOVE=1",
...
void FineTunePanel::handle_pa(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        if (btn == pareset_btn.get_container()) {
            ...
            if (!v.is_null()) {
                gcode_transmit_.gcode_script(fmt::format("SET_PRESSURE_ADVANCE ADVANCE={}", v.template get<double>()));
            }
        } else {
            ...
            if (!cur_pa.is_null()) {
                ...
                gcode_transmit_.gcode_script(fmt::format("SET_PRESSURE_ADVANCE ADVANCE={}", new_pa));
            }
        }
    }
}
...
void FineTunePanel::handle_speed(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        if (btn == speed_reset_btn.get_container()) {
            ...
            gcode_transmit_.gcode_script("M220 S100");
        } else {
            ...
            if (!spd_factor.is_null()) {
                ...
                gcode_transmit_.gcode_script(fmt::format("M220 S{}", new_speed));
            }
        }
    }
}
...
void FineTunePanel::handle_flow(lv_event_t *e)
{
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ...
        if (btn == flow_reset_btn.get_container()) {
            ...
            gcode_transmit_.gcode_script("M221 S100");
        } else {
            ...
            if (!extrude_factor.is_null()) {
                ...
                gcode_transmit_.gcode_script(fmt::format("M221 S{}", new_flow));
            }
        }
    }
}
