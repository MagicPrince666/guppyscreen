...
SettingPanel::SettingPanel(GcodeTransmitClient &c, std::mutex &l, lv_obj_t *parent, SpoolmanPanel &sm)
    : gcode_transmit_(c), cont(lv_obj_create(parent))
#ifndef OS_ANDROID
      ,
      wifi_panel(l)
#endif
      ,
      sysinfo_panel(), spoolman_panel(sm), wifi_btn(cont, &network_img, "WIFI", &SettingPanel::_handle_callback, this), restart_klipper_btn(cont, &refresh_img, "Restart Klipper", &SettingPanel::_handle_callback, this), restart_firmware_btn(cont, &refresh_img, "Restart\nFirmware", &SettingPanel::_handle_callback, this)
#ifdef ZBOLT
      ,
...
#endif
      ,
      spoolman_btn(cont, &spoolman_img, "Spoolman", &SettingPanel::_handle_callback, this), guppy_restart_btn(cont, &refresh_img, "Restart Guppy", &SettingPanel::_handle_callback, this), guppy_update_btn(cont, &update_img, "Update Guppy", &SettingPanel::_handle_callback, this), printer_select_btn(cont, &print, "Printers", &SettingPanel::_handle_callback, this)
{
...
void SettingPanel::handle_callback(lv_event_t *event)
{
    if (lv_event_get_code(event) == LV_EVENT_CLICKED) {
        ...
        if (btn == wifi_btn.get_container()) {
            ...
            gcode_transmit_.send_jsonrpc("printer.restart");
        } else if (btn == restart_firmware_btn.get_container()) {
            spdlog::trace("setting restart klipper pressed");
            gcode_transmit_.send_jsonrpc("printer.firmware_restart");
